  ...
  if (createdCode) {
    return (
      <div className="rounded-2xl border-2 border-brand-200 bg-brand-50 p-5">
        ...
        <p className="mb-2 flex items-center gap-2 font-mono text-lg text-brand-900">
          <KeyRound className="h-4 w-4 text-brand-500" strokeWidth={2} />
          {createdCode.code}
        </p>
  ...
  if (!open) {
    return (
      <button
        ...
        className="flex flex-col items-center justify-center gap-1.5 rounded-2xl border-2 border-dashed border-zinc-300 px-4 py-5 text-sm font-medium text-zinc-500 transition hover:border-brand-300 hover:bg-brand-50/50 hover:text-brand-600"
      >
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-2 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
      <input
        ...
        className="rounded-lg border border-zinc-300 px-3 py-2 text-sm focus:border-brand-400 focus:ring-2 focus:ring-brand-100 focus:outline-none"
      />
      <input
