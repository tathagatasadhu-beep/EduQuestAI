...
export default function TutorChat({ subjectId, subjectName }: { subjectId: string; subjectName: string }) {
  ...
  return (
    <div className="flex flex-col rounded-2xl border-2 border-purple-100 bg-white shadow-sm">
            ...
            <div className="flex flex-col gap-2">
              {SUGGESTED_PROMPTS.map((prompt) => (
                <button
                  key={prompt}
                  onClick={() => send(prompt)}
                  className="flex items-center justify-between rounded-xl bg-purple-50 px-4 py-2.5 text-left text-sm font-medium text-purple-700 transition hover:bg-purple-100"
                >
                  {prompt}
                  <Search className="h-3.5 w-3.5 shrink-0 text-purple-400" strokeWidth={2} />
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            {sending && (
              <div className="flex justify-start">
                <div className="flex items-center gap-1.5 rounded-2xl bg-purple-50 px-4 py-2.5 text-sm text-purple-400">
      ...
      {error && <p className="border-t border-purple-100 px-5 py-2 text-xs text-rose-500">{error}</p>}
      ...
      <form
        ...
        className="flex items-center gap-2 border-t border-purple-100 p-3"
      >
        <input
          value={input}
          ...
          disabled={sending}
          className="flex-1 rounded-full border border-purple-200 px-4 py-2 text-sm focus:border-purple-400 focus:ring-2 focus:ring-purple-100 focus:outline-none disabled:opacity-60"
        />
        <button
          type="submit"
